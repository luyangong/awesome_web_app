...
class RequestHandler(object): # this is the true handler, it calls the functions in handlers.py.
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            if request.method == 'POST':
                if not request.content_type:
                    logging.info('Missing Content-Type')
                    return web.HTTPBadRequest()
                ct = request.content_type.lower()
                if ct.startswith('application/json'):
                    params = await request.json()
                    if not isinstance(params, dict):
                        logging.info('JSON body must be object')
                        return web.HTTPBadRequest()
                    kw = params
                elif ct.startswith('application/x-www-form_urlencoded') or ct.startswith('multipart/form_data'):
...
def add_routes(app, module_name):
    n = module_name.rfind('.') # find the '.' in module_name
    if n == (-1): # if not exist
    ...
    for attr in dir(mod):
        if attr.startswith('_'):
            continue
        fn = getattr(mod, attr) # get the method does not start with '_' from module
        if callable(fn):
            method = getattr(fn, '__method__', None)
            path = getattr(fn, '__route__', None)
            if method and path:
